...
class Clabane: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UIScrollViewDelegate {
    ...
    lazy var refreshControl: UIRefreshControl = {
        ...
        refreshControl.addTarget(self, action: #selector(Clabane.handleRefresh(_:)), for: UIControlEvents.valueChanged)
    ...
    @IBOutlet weak var vinylOutlet: UIBarButtonItem!
    ...
    func fetchPodake (){
        ...
        let task = URLSession.shared.dataTask(with: urlRequets) { (data, response, error) in
            ...
            do {
                ...
                self.whoops()
                print("Da li se ovaj stampa 2 \(error)")
            }
        }
        task.resume()
    }
